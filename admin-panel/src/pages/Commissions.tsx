 ...
 * only how it reads: the ledger sits on a titled surface like every other list
 * in the product, and rates stay ranged right on tabular figures so they can be
 ...
 *
 * THE "EDIT RATE" BUTTON LOST ITS `!px-2 !py-1` OVERRIDE, on both the table and
 * the stacked row. That override computed to a 24px-tall control, and this is a
 * button that opens the editor for what a merchant is charged — a mis-tap next
 * to it lands on the row and does nothing, which trains an operator to jab at
 * it. `.btn` now carries the 44px floor itself, so the override bought nothing
 * except a target a thumb misses.
 */
  ...
  return (
    <>
      <PageHeader
        eyebrow="Merchants"
        title="Commissions"
        description="What each merchant is charged. Every change is saved as a new version and kept for audit."
        meta={
          isLoading
            ? undefined
      ...
      {/* `flush`: the ledger draws its own hairlines edge to edge, and body
          padding would inset the rows from the rules that divide them. */}
      <Section flush title="Rates by merchant">
        <DataTable
          columns={columns}
          rows={rows}
          rowKey={(c) => c.id}
          ...
          // toolbar, so a `70vh` cap is taller than what can be seen.
          maxHeight="70dvh"
          skeletonRows={8}
          pageSize={15}
          renderMobile={(c) => (
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                ...
                <p className="mt-0.5 text-xs capitalize text-slate-500 dark:text-slate-400">
                  ...
                  {c.commission?.feePayer ? ` · ${c.commission.feePayer} pays` : ''}
                </p>
              </div>
              <div className="flex shrink-0 flex-col items-end gap-2 text-right">
                ...
                {/* Full-size, for the same reason the table's copy is: this is
                    the control that opens what a merchant is charged. */}
                <button
                  type="button"
      ...
      footer={
        <>
          ...
          <button
            ...
            onClick={onSubmit}
          >
            {mutation.isPending ? <Spinner size={16} /> : <Save className="h-4 w-4" aria-hidden />}
            Save version
          </button>
        </>
      }
